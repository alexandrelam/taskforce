...
async function runBranchTicketSetup(
  ticketId: string,
  projectPath: string,
  branchName: string,
  postWorktreeCommand: string | null
  ...
  try {
    ...
    const result = createWorktreeFromBranch(projectPath, branchName);
...
app.post("/api/tickets/from-branch", async (req: Request, res: Response) => {
